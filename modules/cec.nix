...
in
{
  ...
  # Kodi must not wake or steal the TV during startup. Once this TV has entered
  # standby, however, it can wake without restoring the Pi as the active CEC
  # source. In that state it stops forwarding remote keys even while showing
  # the Pi's HDMI signal. Arm only after observing standby, then ask the already
  # running Kodi session to activate its source when the TV returns to on.
