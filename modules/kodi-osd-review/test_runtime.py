...
from review_contract import CURRENT_PROPERTY_KEYS, EXPECTED_FOCUS
...
class ReviewRuntimeTest(unittest.TestCase):
    def test_state_clears_then_publishes_ready_last_and_opens_exact_window(self):
        ...
        self.assertEqual(
            harness.window.events[-len(CURRENT_PROPERTY_KEYS) :],
            [("clear", key) for key in CURRENT_PROPERTY_KEYS],
        )
    ...
    def test_close_clears_every_property_and_does_not_open(self):
        ...
        harness.window.values = {
            key: "stale" for key in CURRENT_PROPERTY_KEYS
        }
